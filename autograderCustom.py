...
def readCommand(argv):
    ...
    # csv path to save grades
    parser.add_option('--csv', '-c',
                      dest='csv',
                      default=None,
                      help='Path to save grades in csv format.')
...
def runTest(testName, moduleDict, printTestCase=False, display=None, csv_filename=None):
    import testParser
    import testClasses
    ...
    testCase.execute(grades, moduleDict, solutionDict,
                     csv_filename=csv_filename, test_name=testName)
...
def getDepends(testParser, testRoot, question):
    ...
    questionDict = testParser.TestParser(
        os.path.join(testRoot, question, 'CUSTOMCONFIG')).parse()
...
def getTestSubdirs(testParser, testRoot, questionToGrade):
    problemDict = testParser.TestParser(
        os.path.join(testRoot, 'CUSTOMCONFIG')).parse()
...
def evaluate(generateSolutions, testRoot, moduleDict, exceptionMap=ERROR_HINT_MAP, edxOutput=False, muteOutput=False,
    ...
    for q in test_subdirs:
        ...
        questionDict = testParser.TestParser(
            os.path.join(subdir_path, 'CUSTOMCONFIG')).parse()
...
if __name__ == '__main__':
    ...
    if options.runTest != None:
        runTest(options.runTest, moduleDict, printTestCase=options.printTestCase,
                display=getDisplay(True, options), csv_filename=options.csv)
    else:
